function getDOMArtist(){
    return parseInfo($(".playbackSoundBadge__titleContextContainer>a>span:not(.sc-visuallyhidden)").text()).artist.trim();
}

function getDOMTrack(){
    return parseInfo($(".playbackSoundBadge__titleContextContainer>a>span:not(.sc-visuallyhidden)").text()).track.trim();
}
...
function isDOMTrackAvailable(){
  return $(".playbackSoundBadge__titleContextContainer>a>span:not(.sc-visuallyhidden)").length;
}
...
function getDOMTrackDuration(){
    return hmsToSecondsOnly($(".playbackTimeline__duration>span:not(.sc-visuallyhidden)").text());
}

function getDOMTimeElapsedElement(){
   return $(".playbackTimeline__timePassed>span:not(.sc-visuallyhidden)");
}
